...
{
    if (argc != 3) {
        std::cout << "Usage: triton_benchmark [benchmarks dir] [workload name]" << std::endl;
        exit(-1);
    }
    ...
    benchmark.init(config, tritonClientFactory);
    benchmark.run();
    Json::Value report = benchmark.generateReport();
    std::cout << "\nreport:\n" << report << std::endl;
    std::string resultPath = joinPath(joinPath(benchmarksDir, "results"), std::string("triton.") + argv[2] + ".json");
